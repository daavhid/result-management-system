...
const ManageStudents = () => {
  ...
  return (
    <div className='border border-tone6 m-8 mt-0 bg-white rounded-xloverflow-hidden'>
        <p className='px-8 py-4 border-b text-xl text-gray-600 font-semibold'>Students</p>
        <ScrollArea className='max-h-[20rem]  overflow-hidden' >
            <Table className=' mx-auto relative z-20 '>
                <TableHeader >
                    <TableRow className='text-center'>
                        ...
                        <TableHead className=' uppercase  text-sm'>Matric Number</TableHead>
                ...
                <TableBody>
                    {results?.map((data:any, index:number) => (
                        <TableRow className={cn(index%2===1&&'bg-blue-50  hover:bg-blue-50')} key={index}>
                            ...
                            <TableCell className='uppercase font-medium'>{data?.matric_number}</TableCell>
                            <TableCell className=' font-medium'>{data?.email}</TableCell>
                            <TableCell className='uppercase font-medium'>{data?.departments?.departmentCode}</TableCell>
