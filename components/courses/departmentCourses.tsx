...
import { getCourses, getDepartmentStaffs, getSemester, getStaff } from '@/actions/general'
...
const DepartmentCourses = ({user}:{user:any}) => {
    ...
    useEffect(()=>{
        const fetch = async()=>{
            ...
            // const semester = await axios.get(`/api/semester?search=${new Date()}`)
            const coursesdata = await getCourses(department)
            setCourses(prev=>coursesdata?.map((course:any)=>({value:course?.courseCode})))
            // const coursesdata = await axios.get(`/api/courses?search=${department}`)
            const staffData = await getDepartmentStaffs(department)
            ...
            // setSemester(prev=>[{value:semester.data?.semester?.semesterCode}])
            setStaffs(prev=>staffData?.map((staff:any)=>({name:`${staff?.title} ${staff?.firstName.charAt(0)} ${staff?.lastName}`,id:staff?.id})))
    ...
    console.log(courses,'this is the Course')
    console.log(department,'this is the department')
