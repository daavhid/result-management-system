...
import { getSemester, staffDepartmentCourses } from '@/actions/general'
...
const Result = ({user}:{user:any})=>{
    ...
    useEffect(()=>{
        const fetch = async()=>{
            ...
            const data = await axios.get(`/api/staffs?email=${user?.email}`)
            setStaff(data.data?.staff)
            console.log(data.data?.staff)
            const coursesData = await staffDepartmentCourses(data.data?.staff?.id,semesterCode)
    ...
    const sendResult = async()=>{
        try {
            ...
            if(resultData.error){
                toast.success(`${resultData?.error}`)
            }
        } catch (error) {
            console.log(error)
        }
    ...
    return(
        <div className=' grid grid-cols-3 space-x-6'>
            <div className='border relative bg-white h-[380px]  col-span-2 rounded-xl shadow-2xl'> 
                <Form {...form}>
                    ...
                    className=' bg-white  p-4 flex flex-col mt-16 gap-2 rounded-lg' >
                            ...
                            render={({field})=>(
                                <FormItem>
                                    ...
                                    <Select onValueChange={field.onChange} onOpenChange={()=>{setCourse(field.value)}} value={field.value}>
                                        <SelectTrigger className=' ring-0 border-t-0 border-x-0 border-b-2 outline-none focus-visible:ring-0 focus-visible:ring-offset-0 border-b-gray-300 '>
                        ...
                        <div className='flex space-x-4 items-center mt-10'>
                                    ...
                                    <Popover >
                                        <PopoverTrigger asChild>
                                            <FormControl className=''>
                                                ...
                                                role="combobox"
                                                aria-expanded={true}
                                                className="  border-t-0 border-x-0 w-40 text-left"
                                                >
                                                {field.value
                            ...
                            render={({field})=>(
                                <FormItem>
                                    <FormControl >
                                        <Input  className=' w-36 ring-0 border-t-0 border-x-0 border-b-2 outline-none focus-visible:ring-0 focus-visible:ring-offset-0 border-b-gray-300 ' placeholder='Enter C.A Score' type='number'  {...field} onKeyDownCapture={(e) =>["e", "E", "+", "-"].includes(e.key) && e.preventDefault()} min={0} max={40} pattern='\d{2}' />
                                    </FormControl>
                                    <FormMessage/>
                                </FormItem>
                            )}/>
                            ...
                            render={({field})=>(
                                <FormItem>
                                    ...
                                    <FormControl >
                                        <Input  className=' w-36 ring-0 border-t-0 border-x-0 border-b-2 outline-none focus-visible:ring-0 focus-visible:ring-offset-0 border-b-gray-300 ' placeholder='Enter Exam Score' type='number'  {...field} onKeyDownCapture={(e) =>["e", "E", "+", "-"].includes(e.key) && e.preventDefault()} min={0} max={60} pattern='\d{2}' />
                                    </FormControl>
                                    <FormMessage/>
                                </FormItem>
                            )}/>
                            <Button type='submit' variant={'ghost'} className=' w-fit rounded-md    bg-green-600 z-40 text-white ' >Add Result</Button>
                        </div>
                    </form>
            </Form>
            <Button type='submit' variant={'ghost'} onClick={()=>{
                sendResult()
            }} className='   rounded-md    bg-purple-600 z-40 text-white absolute bottom-4 left-4'>Send Result</Button>
