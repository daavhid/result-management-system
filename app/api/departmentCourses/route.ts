...
export async function POST(req:NextRequest,){
    ...
    try {
        ...
        if(existingdeptCourse) {
            return NextResponse.json({error: "departmental course already exists"},{status:405})
        }
        if(!existingCourse){
        ...
        return NextResponse.json({message:'Staff Assigned to course Successfully'},{status:201})
