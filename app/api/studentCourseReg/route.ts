    ...
    try {
        for(let id of departmentCourses){
            ...
            if(passed){
                ...
                // if(semesterId === existingStudentCourseReg.semesterId){
                //     return NextResponse.json({error:'cannot register for the same course in a single semester'},{status:404})
                // }
                // 
                //         // await db.results.update({
                //         //     where:{
                //         //         id:existingCourseResult.id,
                //         //     },data:{
                //         //         isRetake:0
                //         //     }
                //         // })
                //         return NextResponse.json({newStudentCourseReg},{status:201})
        
                //     }else{
                //         return NextResponse.json({error:"course has been registered and passed"})
                //     }
